...
const modules = [
  ...
  { num: "04", title: "Engenharia do Discurso", desc: "Como transformar dados técnicos em significado." },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2.5">
          {modules.map((mod, i) => (
            ...
            >
              ...
              <h3 className="text-sm font-body font-bold text-foreground mb-0.5">{mod.title}</h3>
              <p className="text-muted-foreground font-card text-xs leading-relaxed">{mod.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
